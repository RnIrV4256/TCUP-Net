...
class DatasetFromFolder3D(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if ((np.max(labed_img) - np.min(labed_img))) == 0:
            print(random_index, self.labeled_filenames[random_index])
        ...
        labed_lab = np.tile(labed_lab, (2, 1, 1)) / 255.
        ...
        # unlabed_img = np.where(unlabed_img < 0., 0., unlabed_img)
        # unlabed_img = np.where(unlabed_img > 2048., 2048., unlabed_img)
        # unlabed_img = unlabed_img / 2048.
        if ((np.max(unlabed_img) - np.min(unlabed_img))) == 0:
            print(random_index, self.unlabeled_filenames[random_index])
