...
class DatasetFromFolder3D(data.Dataset):
    ...
    def __getitem__(self, index):
        labed_name = self.labeled_filenames[index]
        labed_h5f = h5py.File(self.labeled_file_dir + "/" + labed_name + "/mri_norm2.h5", 'r')
        labed_img = labed_h5f['image'][:]
        labed_lab = labed_h5f['label'][:]
        labed_img, labed_lab = self.RandomCrop(labed_img, labed_lab)
        # labed_img = self.pad(labed_img)
        labed_img = labed_img.astype(np.float32)
        labed_img = labed_img[np.newaxis, :, :, :]
        # labed_lab = self.pad(labed_lab)
        labed_lab = self.to_categorical(labed_lab, self.num_classes)
        labed_lab = labed_lab.astype(np.float32)


        return labed_img, labed_lab, labed_name
